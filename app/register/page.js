...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND}/src/auth/register`, {
        method: 'POST',
      ...
      <main className="container mx-auto flex-1 p-4">
        ...
        <form onSubmit={handleSubmit} className="bg-white p-4 rounded-lg shadow-lg space-y-4">
          ...
          <div>
            ...
            <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full border border-gray-300 rounded p-2" />
          </div>
          <div>
          ...
          <div>
            <button type="submit" className="bg-purple-500 text-white py-2 px-4 rounded-full hover:bg-purple-600 transition duration-300">Registrar</button>
          </div>
        </form>
      </main>
    </div>
